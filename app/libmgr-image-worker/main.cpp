...
int main(int argc, char* argv[]){
  ...
  for(;;){ // reconnect loop
    try{
      ...
      std::cout << R"({"level":"info","msg":"image-worker connected"})" << "\n";
        ...
        if(type == "PingRequest"){
          ...
          try {
            ...
            if(!fs::exists(p) || !fs::is_regular_file(p)){
              ...
              // run image analysis helper
              auto meta = handle_image_file(0, req.path);
              ...
              // build JSON with standard + image fields
            ...
            fe.error=e.what();
            ws.write(asio::buffer(nlohmann::json(fe).dump()));
            std::cerr << "[ImageWorker] Sent FileError: " << e.what() << "\n";
          }
        }
      } // end message loop

    }catch(const std::exception& e){
      std::cerr << R"({"level":"error","msg":"image-worker disconnected","err":")"
                << e.what() << "\"}\n";
